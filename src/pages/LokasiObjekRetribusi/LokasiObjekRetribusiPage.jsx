// src/pages/LokasiObjekRetribusi/LokasiObjekRetribusiPage.jsx
...
import MoreVertIcon from '@mui/icons-material/MoreVert'; // Untuk menu aksi tabel
import SearchIcon from '@mui/icons-material/Search';     // Untuk kolom pencarian
import FilterListIcon from '@mui/icons-material/FilterList'; // Untuk tombol filter
import SaveIcon from '@mui/icons-material/Save';             // Untuk tombol simpan
import ArrowBackIcon from '@mui/icons-material/ArrowBack';   // Untuk tombol kembali
...
const LokasiObjekRetribusiPage = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    setPageError(null);
    try {
      const response = await apiClient.get('/lokasi-objek-retribusi');
      setDataList(response.data.data || []);
    } catch (err) {
      setPageError('Gagal mengambil data Lokasi Objek Retribusi.');
  ...
  const handleSetFormMode = (mode, item = null) => {
    handleCloseMenu(); // Tutup menu jika ada yang terbuka
    setEditingItem(item);
    if (mode === 'edit' && item) {
      setFormData({
        idLokasiObjekRetribusi: item.idLokasiObjekRetribusi,
        lokasiObjekRetribusi: item.lokasiObjekRetribusi || '',
  ...
  const handleSubmit = async () => {
    if (!formData.lokasiObjekRetribusi) {
        setFormError("Lokasi Objek Retribusi wajib diisi.");
        return;
    }
    const submitButton = document.getElementById('submit-button-lokasi-objek-retribusi');
    if(submitButton) submitButton.disabled = true;
    setFormError(null);

    try {
      const payload = { ...formData };
      if (formMode === 'edit' && editingItem && formData.idLokasiObjekRetribusi) {
        await apiClient.put(`/lokasi-objek-retribusi/${formData.idLokasiObjekRetribusi}`, payload);
        showSnackbar('Lokasi Objek Retribusi berhasil diperbarui!', 'success');
      } else {
        const { idLokasiObjekRetribusi, ...createData } = payload; // Hapus ID jika add mode
        await apiClient.post('/lokasi-objek-retribusi', createData);
        showSnackbar('Lokasi Objek Retribusi baru berhasil ditambahkan!', 'success');
      }
      setFormMode('list'); // Kembali ke daftar setelah sukses
      fetchData(); // Muat ulang data
    } catch (err) {
      console.error("Error saat menyimpan Lokasi Objek Retribusi:", err);
  ...
  const handleDeleteConfirmation = (itemId) => {
    handleCloseMenu();
    if (window.confirm('Apakah Anda yakin ingin menghapus lokasi objek retribusi ini?')) {
      handleDelete(itemId);
    }
  };
  ...
  const handleDelete = async (id) => {
    setPageError(null);
    try {
      await apiClient.delete(`/lokasi-objek-retribusi/${id}`);
      fetchData();
      showSnackbar('Lokasi Objek Retribusi berhasil dihapus.', 'success');
    } catch (err) {
      console.error("Error menghapus Lokasi Objek Retribusi:", err);
      setPageError('Gagal menghapus data.');
      showSnackbar('Gagal menghapus data.', 'error');
    }
  };

  const tableHeaders = ["ID", "Lokasi Objek Retribusi", "Keterangan", "Aksi"];
        ...
        <Paper sx={{ 
            ...
            <Box sx={{ 
                ...
                <Typography variant="h6" component="h1" sx={{ fontWeight: 'medium' }}>
                    {formMode === 'edit' ? 'Edit Data' : 'Formulir Tambah'} Lokasi Objek Retribusi
                </Typography>
            </Box>
            ...
            <Grid container spacing={2.5}>
              <Grid item xs={12}>
                <Typography variant="subtitle2" gutterBottom sx={{fontWeight: 'bold', color: 'text.primary', mb: 0.5}}>Lokasi Objek Retribusi *</Typography>
                <TextField
                    autoFocus
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <Button 
                id="submit-button-lokasi-objek-retribusi" 
                ...
                startIcon={<SaveIcon />}
              >
                {formMode === 'edit' ? 'Simpan Perubahan' : 'Simpan Lokasi'}
              </Button>
              <Button 
  ...
  return (
    <Box sx={{ p: { xs: 1, sm: 2, md: 3 } }}>
      <Paper sx={{ 
        ...
        <Box sx={{ display: 'flex', flexDirection: {xs: 'column', sm: 'row'}, justifyContent: 'space-between', alignItems: {sm:'center'}, gap: 2, mb: 2.5 }}>
          <Typography variant="h4" component="h1" sx={{ fontWeight: 'bold', color: theme.palette.text.primary }}>
            Manajemen Lokasi Objek Retribusi
          </Typography>
          <Button 
            variant="contained" 
            startIcon={<AddIcon />} 
        ...
        <Box sx={{display: 'flex', gap: 1, mb: 2}}>
            <TextField size="small" placeholder="Cari Lokasi..." InputProps={{startAdornment: <SearchIcon fontSize="small" sx={{mr:0.5, color:'text.disabled'}}/>}}/>
          ...
          <Table stickyHeader sx={{ minWidth: 650 }}>
            ...
            <TableBody>
              {loading ? <TableSkeleton /> : 
                dataList.length === 0 ? (
                  <TableRow><TableCell colSpan={tableHeaders.length} align="center" sx={{py:5}}>Tidak ada data lokasi objek retribusi.</TableCell></TableRow>
              ) : (
                dataList.map((item) => (
                <TableRow hover key={item.idLokasiObjekRetribusi} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                    <TableCell>{item.idLokasiObjekRetribusi}</TableCell>
                    <TableCell sx={{ fontWeight: 500 }}>{item.lokasiObjekRetribusi}</TableCell>
                    <TableCell>{item.keterangan || '-'}</TableCell>
                    <TableCell align="right">
                      <IconButton size="small" onClick={(e) => handleOpenMenu(e, item.idLokasiObjekRetribusi)}>
                        <MoreVertIcon />
                      </IconButton>
                    </TableCell>
                </TableRow>
      ...
      >
        <MenuItem onClick={() => handleSetFormMode('edit', dataList.find(d => d.idLokasiObjekRetribusi === selectedItemIdMenu))}>
