// src/pages/JenisStatus/JenisStatusPage.jsx
...
const JenisStatusPage = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    setPageError(null);
    try {
      const response = await apiClient.get('/jenis-status');
      setDataList(response.data.data || []);
    } catch (err) {
      setPageError('Gagal mengambil data Jenis Status.');
      console.error("Error fetching Jenis Status:", err);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleSubmit = async () => {
    if (!formData.jenisStatus) {
        setFormError("Jenis Status wajib diisi.");
        return;
    }
    const submitButton = document.getElementById('submit-button-jenis-status-page'); // ID Unik
    if(submitButton) submitButton.disabled = true;
    setFormError(null);

    try {
      const payload = { ...formData };
      if (formMode === 'edit' && editingItem && formData.idJenisStatus) {
        await apiClient.put(`/jenis-status/${formData.idJenisStatus}`, payload);
        showSnackbar('Jenis Status berhasil diperbarui!', 'success');
      } else {
        const { idJenisStatus, ...createData } = payload;
        await apiClient.post('/jenis-status', createData);
        showSnackbar('Jenis Status baru berhasil ditambahkan!', 'success');
      }
      setFormMode('list');
      fetchData();
    } catch (err) {
      console.error("Error saat menyimpan Jenis Status:", err);
  ...
  const handleDelete = async (id) => {
    setPageError(null);
    try {
      await apiClient.delete(`/jenis-status/${id}`);
      fetchData();
      showSnackbar('Jenis Status berhasil dihapus.', 'success');
    } catch (err) {
      console.error("Error menghapus Jenis Status:", err);
      setPageError('Gagal menghapus data.');
      showSnackbar('Gagal menghapus data.', 'error');
    }
  };

  const tableHeaders = ["ID", "Jenis Status", "Keterangan", "Aksi"];
        ...
        <Paper sx={{ 
            ...
            <Box sx={{ 
                ...
                <Typography variant="h6" component="h1" sx={{ fontWeight: 'medium' }}>
                    {formMode === 'edit' ? 'Edit Data' : 'Formulir Tambah'} Jenis Status
                </Typography>
            </Box>
            ...
            <Grid container spacing={2.5}>
              <Grid item xs={12}>
                ...
                <TextField
                    ...
                    required
                    size="small"
                    placeholder="Masukkan nama jenis status"
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <Button 
                id="submit-button-jenis-status-page" 
                ...
                startIcon={<SaveIcon />}
              >
                {formMode === 'edit' ? 'Simpan Perubahan' : 'Simpan Jenis Status'}
              </Button>
              <Button 
    ...
    <Box sx={{ p: { xs: 1, sm: 2, md: 3 } }}>
      <Paper sx={{ 
        ...
        <Box sx={{display: 'flex', gap: 1, mb: 2}}>
            <TextField size="small" placeholder="Cari Jenis Status..." InputProps={{startAdornment: <SearchIcon fontSize="small" sx={{mr:0.5, color:'text.disabled'}}/>}}/>
          ...
          <Table stickyHeader sx={{ minWidth: 650 }}>
            ...
            <TableBody>
              {loading ? <TableSkeleton /> : 
                dataList.length === 0 ? (
                  <TableRow><TableCell colSpan={tableHeaders.length} align="center" sx={{py:5}}>Tidak ada data jenis status.</TableCell></TableRow>
              ) : (
                dataList.map((item) => (
                <TableRow hover key={item.idJenisStatus} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                    <TableCell>{item.idJenisStatus}</TableCell>
                    <TableCell sx={{ fontWeight: 500 }}>{item.jenisStatus}</TableCell>
                    <TableCell>{item.keterangan || '-'}</TableCell>
                    <TableCell align="right">
                      <IconButton size="small" onClick={(e) => handleOpenMenu(e, item.idJenisStatus)}>
                        <MoreVertIcon />
                      </IconButton>
                    </TableCell>
                </TableRow>
      ...
      >
        <MenuItem onClick={() => handleSetFormMode('edit', dataList.find(d => d.idJenisStatus === selectedItemIdMenu))}>
